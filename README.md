...
└── asterisk/
    ...
    └── scripts/
        ├── notify*.sh            ← 伝言の通知
        ├── cleanup-recordings.sh ← 古い録音の自動削除
        └── analyze-recording.py  ← 音声品質の解析（→ 9章）
```
...
## 9. 音声品質の調査（両方向を別トラックで録音する）

「相手に聞こえづらい」「同時にしゃべると聞こえなくなる」を**推測ではなく数値で**
切り分けるための機能。通話音声を **行き(tx) / 帰り(rx) に分けて** 録音し、
付属スクリプトでレベルとダブルトーク時の落ち込みを解析する。

> ミックス1本だけ録っても「どちらの声が消えているか」が判別できないため、
> 必ず別トラックで録る。ここが調査の肝。

### 9.1 有効にする

```
# 全通話を録音（.env）。調査中だけ 1 にする
QA_RECORD=1
QA_RETENTION_DAYS=7        # qa/ の保存日数。容量を食うので短め

docker compose up -d       # .env は起動時に読まれる
```

起動後は **Linphoneからダイヤルするだけで切替**できる（再起動不要・こちらが優先）:
...
`./recordings/qa/` に1通話あたり3本:

```
20260818-101530_out_5105_1755123.wav       両方向ミックス（耳で聴く用）
20260818-101530_out_5105_1755123-rx.wav    録音チャネルが「受け取った」音声
20260818-101530_out_5105_1755123-tx.wav    録音チャネルへ「送った」音声
```

ファイル名の `_out_` / `_in_` が方向。どちらが自分の声かは経路で入れ替わる:

| | rx | tx |
| --- | --- | --- |
| `_out_`（Linphoneから発信） | **自分(Linphone)の声** | 相手(PBX)の声 |
| `_in_`（外線着信） | 相手(PBX)の声 | **自分(Linphone)の声** |

> 迷ったら **内線296を1回録る**。BGMが入っているほうが「相手の声」トラック。

留守電の伝言録音（`./recordings/*.wav`）とは**別ディレクトリ**なので、
Discord/メール通知や内線202の対象にはならない。

### 9.3 切り分け用のテスト内線

| 内線 | 内容 |
| --- | --- |
| `295` | 基準取り。ビープ後に10秒ふつうに話す。**単独発話時**の自分の音量が分かる |
| `296` | **ダブルトーク試験**。最初の10秒は黙って話し、ビープ後はBGM（=相手役）に**重ねて**話す |

**296が決定打**。PBXもHT813も一切通らないので、ここで自分の声が削られていれば
犯人は **Linphone(スマホ)側のエコーキャンセラ/AGC** で確定する。
逆に296で問題がなければ、原因は HT813のFXO か PBX 側。

### 9.4 解析する

ホスト側で実行（追加インストール不要。Python3標準ライブラリのみ）:

```bash
# 最新の1通話を解析
python3 asterisk/scripts/analyze-recording.py recordings/qa

# ファイル指定（-rx/-tx/拡張子はあってもなくてもよい）
python3 asterisk/scripts/analyze-recording.py recordings/qa/20260818-101530_out_5105_1755123.wav
...
[ダブルトーク解析] 自分だけ話しているとき vs 同時に話しているとき
  単独発話中の自分のレベル :   -13.5 dBFS   (5.0秒)
  同時発話中の自分のレベル :   -31.7 dBFS   (6.0秒)
  → 同時発話時のレベル低下 : +18.2 dB

[所見]
  ★ 同時発話でこちらの声が 18dB 削られています。半二重動作が原因です。
```
...
| 指標 | 目安 | 疑うところ |
| --- | --- | --- |
| 発話中の平均レベル | **-30dBFSより小さい** | 送出音量不足。マイク音量 / HT813のFXOゲイン |
| 同時発話時のレベル低下 | **6dB以上** | エコーキャンセラの半二重動作 |
| 途切れ(ドロップ)率 | **25%超** | Wi-Fi/パケットロス、VAD(無音圧縮) |
| クリップ率 | **1%以上** | 音量の上げすぎ。歪んで逆に聞き取れない |

### 9.5 「同時にしゃべると聞こえない」の対策順

症状は典型的な **半二重（片方が話すともう片方が切られる）**。
原因は3か所のどれかなので、上から順に潰す。

1. **Linphone側（最有力・296で判定できる）**
   - **有線ヘッドセットを使う**。スピーカーホン運用だとエコーキャンセラが
     常時強く働き、同時発話でこちらの声を丸ごと消す。これだけで直ることが多い。
   - Linphone の設定で **エコーキャンセラ** をオフにして試す
     （ヘッドセットなら物理的な回り込みが無いので不要）。
   - **AGC / ノイズ抑制** もオフにして比較する。
   - Bluetoothイヤホンは自前のAECが二重にかかるので、切り分け中は使わない。

2. **HT813 の FXO側（296で問題が出なければここ）**
   FXO は 4線(VoIP) ↔ 2線(アナログ) を変換する。ここのインピーダンス不整合で
   エコーが返り、それを消そうとして半二重になる。FXO PORT タブで:
   - **SLIC Setting / Impedance** を日本向け（**600Ω**）に。既定の米国設定だと
     ハイブリッドが合わず、エコーが大量に出る。**最優先で確認**。
   - **Silence Suppression / VAD = No**（小さい声が丸ごと送られなくなる）
   - **Echo Cancellation** を Yes/No で両方試す。PBX側にもECがある場合、
     二重にかかって余計に削れることがある。
   - **AGC = 無効**（レベルが揺れて聞き取りづらくなる）
   - **FXO Transmit/Receive Gain** を ±3dB ずつ動かして 9.4 で再測定。
     上げすぎるとクリップするので、必ず数値を見ながら。
   - 優先コーデックは **PCMU/PCMA固定**（G.729等はダブルトークに弱い）。

3. **経路・PBX側**
   - スマホを **有線と同じ帯のWi-Fi** に。ゲストSSID・APアイソレーションは論外。
   - `docker compose logs` と `pjsip set logger on` でパケットロスを確認。
   - ジッタが大きいなら extensions.conf の Dial の前に
     `same => n,Set(JITTERBUFFER(adaptive)=default)` を足す。
     ただし遅延が増えるとエコーは悪化するので、必要なときだけ。
   - PBX/相手電話機側のエコーサプレッサは触れないことが多い。
     その場合は 1 と 2 を詰めるしかない。

### 9.6 応急処置: 送出音量だけ上げる

原因究明の前に「とりあえず聞こえるように」したい場合、`extensions.conf` の
`[globals]` にある `QA_GAIN` に値を入れる（`-4`〜`4` 程度。空=無効）。
...
> ★ **測定するときは必ず空に戻す**こと。ゲイン補正を入れたまま解析すると
> 本来のレベルが分からなくなる。あくまで暫定策で、根治は 9.5 で行う。

### 9.7 調査が終わったら

```
QA_RECORD=0     # .env（または内線292）
```

録音は通話ごとにディスクを食う（8kHz/16bit × 3本 ≒ 3MB/分）。
`QA_RETENTION_DAYS` 日を過ぎたものは cleanup コンテナが自動削除する。

> **注意**: 通話録音は相手の音声も記録される。社内ルール・相手への告知など、
> 運用上の扱いは各自で確認すること。調査が済んだら必ずOFFに戻す。
